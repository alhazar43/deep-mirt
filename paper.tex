...
In this paper we instantiate the head with the GPCM over the other major polytomous families for three reasons: (1) it is an adjacent-category model, which is the natural match for rubric scoring where a student progresses sequentially through performance levels; (2) it produces a discrimination parameter $\alpha_j$ and ordered step difficulty thresholds $\boldsymbol{\beta}_j$ that correspond directly to rubric design decisions and can be inspected by assessment practitioners; (3) its monotone threshold constraint admits a clean differentiable parameterization (Section~\ref{sec:gpcm_head}). A cumulative model such as the GRM~\citep{samejima_estimation_1969} would be a natural alternative for tasks where each scoring threshold represents an independent decision (e.g., analytic rubrics with independent sub-scores); a NRM~\citep{bock_estimating_1972} would apply when response categories are unordered (e.g., multiple-choice representing different misconceptions). We treat these as directions for future instantiation.
...
We simulate $5000$ students interacting with questions drawn from a pool of $Q=200$ questions in random order. Each interaction sequence has variable length in $[50,200]$. Students and questions are configured with \emph{static} GPCM parameters following assumed priors:
\begin{itemize}
...
We compare DEEP-GPCM against four baselines that progressively relax psychometric and ordinal structure, each isolating a different design question:
\begin{itemize}
...
Table~\ref{tab:comp_results} reports prediction performance on Synthetic-Ordinal for $K\in\{2,3,4,5\}$, addressing \textbf{RQ1}.

\begin{table*}[htb!]
\centering
\caption{Prediction performance on Synthetic-Ordinal ($Q=200$) across $K\in\{2,3,4,5\}$. Best per column in bold.}
...
\midrule
Static GPCM & 61.6 & 0.664 \\
Dynamic GPCM & 68.5 & 0.753 \\
DKVMN + Softmax & 70.33 & 0.776 \\
DKVMN + Ordinal & 70.59 & 0.778 \\
\midrule
DEEP-GPCM (ours) & \textbf{71.4} & \textbf{0.779} \\
\bottomrule
\end{tabular}
\end{subtable}
\hfill
...
\midrule
Static GPCM & 48.5 & 0.313 & 0.233 \\
Dynamic GPCM & 57.2 & 0.555 & 0.407 \\
DKVMN + Softmax & 60.18 & 0.570 & 0.407 \\
DKVMN + Ordinal & 59.59 & 0.592 & 0.431 \\
\midrule
DEEP-GPCM (ours) & \textbf{60.7} & \textbf{0.618} & \textbf{0.567} \\
\bottomrule
\end{tabular}
\end{subtable}
...
\textbf{Model} & \textbf{Acc(\%)} & \textbf{QWK} & $\boldsymbol{\tau}$ \\
\midrule
Static GPCM & 39.0 & 0.322 & 0.244 \\
Dynamic GPCM & 49.6 & 0.644 & 0.490 \\
DKVMN + Softmax & 53.58 & 0.650 & 0.482 \\
DKVMN + Ordinal & 52.58 & 0.684 & 0.516 \\
\midrule
DEEP-GPCM (ours) & \textbf{53.7} & \textbf{0.710} & \textbf{0.627} \\
\bottomrule
\end{tabular}
\end{subtable}
\hfill
...
\textbf{Model} & \textbf{Acc(\%)} & \textbf{QWK} & $\boldsymbol{\tau}$ \\
\midrule
Static GPCM & 37.4 & 0.390 & 0.298 \\
Dynamic GPCM & 45.3 & 0.717 & 0.553 \\
DKVMN + Softmax & \textbf{51.73} & 0.724 & 0.541 \\
DKVMN + Ordinal & 50.13 & \textbf{0.762} & \textbf{0.582} \\
\midrule
DEEP-GPCM (ours) & 50.7 & 0.759 & 0.576 \\
\bottomrule
\end{tabular}
\end{subtable}
\end{table*}

Two findings stand out. First, the $\tau$ advantage of DEEP-GPCM over the DKVMN baselines is largest at $K=3$ and shrinks as $K$ grows, reaching near-parity at $K=5$. This is counterintuitive in terms of GPCM's ordinal structure where more thresholds should, if anything, help more at higher $K$. A feasible explanation is that at low $K$, the ordinal signal per observation is weak and the GPCM's ability-centred parameterization provides a stronger inductive bias for ranking students correctly; at high $K$, the richer per-step signal allows the DKVMN baselines to learn comparable rank structure from data alone. Second, the IRT ablation shows that sequential dynamics, not item parameterization, drive the bulk of performance: Dynamic GPCM, which shares item tables with Static GPCM but adds a single gated ability update, recovers most of the gap to the neural models. At $K=4$ and $K=5$, Dynamic GPCM nearly matches DKVMN + Softmax on QWK despite having no distributed memory, which suggests that a lightweight recurrent IRT model is a surprisingly competitive baseline when the response scale is fine-grained enough to provide a strong per-step learning signal.

\subsubsection{Binary Compatibility on Standard KT Benchmarks}
For practical deployment, it is important for PKT models to not sacrifice performance even when tested on binary data, since many existing item banks and datasets use dichotomous scoring, and a useful framework must handle both formats without architectural modification. We evaluate backward compatibility by instantiating DEEP-MIRT with $K=2$ and testing on ASSIST2015 and Synthetic-5. Results are reported in Table~\ref{tab:combined_perf}. DEEP-GPCM achieves competitive performance compared to DKT and DKVMN despite substituting the binary readout with a GPCM head, confirming that the GPCM collapses to a standard logistic model when $K=2$.

\begin{table}[htb!]
\centering
\caption{Binary compatibility on ASSIST2015 and Synthetic-5 ($K=2$). Best in bold.}
...
Figure~\ref{fig:trajectories} compares learner state trajectories across three models for four synthetic types of students. At each time $t$, we project $\theta_t$ to responses with the final output head of each model for the reference item. Therefore a faster convergence to the true projected category suggests higher confidence in modeling learning dynamics.

Specifically, dynamic GPCM's $\theta_t$ is a per-student embedding updated by a fixed recurrence, producing trajectories that are smooth but relatively flat, which means that the model adjusts slowly and converges to a stable estimate regardless of archetype. DEEP-GPCM and DKVMN+Softmax, sharing the same DKVMN backbone, produce trajectories by tracking the local response pattern, which yields fast convergence to projected responses even with the mid-ability type.

\begin{figure}[htb!]
\centering
\vspace{-8pt}
\input{kt-gpcm/outputs/deepgpcm_k4_s42/trajectory_plots/learner_trajectories.pgf}
\vspace{-6pt}
\caption{Learner state trajectories for four archetypes ($K=4$, $Q=200$). Curves smoothed with a 9-step rolling mean.}
\label{fig:trajectories}
\end{figure}

The similarity between DEEP-GPCM's $\theta_t$ and DKVMN+Softmax's expected score $\hat{s}_t$ is itself informative. Although both curves track each other closely across all four types, the trajectory reading from the GPCM head is able to output prediction on psychometrically interpretable scales, without losing the latent signal from the memory. The practical implication is that replacing a softmax head with a GPCM head costs nothing in trajectory quality while gaining insights to item-level characteristics, deriving richer context that $\hat{s}_t$ alone cannot support.

\subsubsection{IRT Parameter Recovery and Stability (RQ3)}
\label{sec:irt_analysis}
DEEP-GPCM recovers the discrimination parameter, $\alpha$, more consistently than both GPCM baselines across most conditions ($r_\alpha = 0.75$--$0.80$ vs.\ $0.69$--$0.76$ for Static and Dynamic GPCM; Table~\ref{tab:irt_recovery_k}), despite receiving no direct supervision. This is suggests the DKVMN memory, trained solely on ordinal prediction, can distinguish item-level characteristics. Step difficulty recovery is slightly lower ($\bar{r}_\beta = 0.82$--$0.92$ for DEEP-GPCM vs.\ $0.97$--$0.99$ for the baselines), which is expected as both GPCM bas dedicated lookup tables, offering an unobstructed gradient path to each $\beta$, whereas DEEP-GPCM infers thresholds as a byproduct of sequential prediction.

Figure~\ref{fig:recovery_student} shows that the estimated ability distributions closely track the $\mathcal{N}(0,1)$ generating prior, confirming population-level calibration. Low $r_\theta$ is expected, as the synthetic ground truth is a static scalar, whereas $\theta_t$ is a dynamic trajectory that updates with each interaction. A meaningful criterion for a dynamic estimator is whether $\theta_t$, or projected response category converge at later interactions.

\begin{figure}[t]
\centering
\vspace{-8pt}
\input{kt-gpcm/outputs/deepgpcm_k4_s42/recovery_split_student.pgf}
\vspace{-6pt}
\caption{Student ability recovery ($K=4$, $Q=200$, $N=5000$). Each row shows one model's estimated $\theta$ distribution compared to ground truth.}
\label{fig:recovery_student}
\end{figure}

Figure~\ref{fig:recovery_item} shows item parameter recovery. Static GPCM: $r_\alpha=0.606$, $\bar{r}_\beta=0.983$. Dynamic GPCM: $r_\alpha=0.700$, $\bar{r}_\beta=0.966$. DEEP-GPCM: $r_\alpha=0.707$, $\bar{r}_\beta=0.918$.

\begin{figure*}[t]
\centering
\vspace{-8pt}
\resizebox{\linewidth}{!}{\input{kt-gpcm/outputs/deepgpcm_k4_s42/recovery_split_item.pgf}}
\vspace{-6pt}
\caption{Item parameter recovery ($K=4$, $Q=200$, $N=5000$). Each row shows one model; columns show discrimination $\alpha$ (log-linked) and step difficulties $\beta_1$--$\beta_3$ (z-scored). Pearson $r$ is annotated per panel.}
\label{fig:recovery_item}
\end{figure*}

Table~\ref{tab:irt_recovery_k} reports the full breakdown across $K \in \{3,4,5\}$; taken together, DEEP-GPCM achieves competitive recovery of both item parameters without any direct IRT supervision.

\begin{table}[htb!]
\centering
\caption{IRT parameter recovery across models and $K$ ($Q=200$, $N=5000$). $r_\beta$ is the mean across all $K{-}1$ thresholds.}
\label{tab:irt_recovery_k}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccc}
\toprule
$K$ & Model & $r_\alpha$ & $r_\beta$ (mean) & Per-threshold correlation \\
\midrule
\multirow{3}{*}{3}
 & Static GPCM  & 0.732 & \textbf{0.989} & $r_{\beta_1}{=}0.990$, $r_{\beta_2}{=}0.987$ \\
 & Dynamic GPCM & 0.694 & 0.976 & $r_{\beta_1}{=}0.976$, $r_{\beta_2}{=}0.975$ \\
 & DEEP-GPCM    & \textbf{0.777} & 0.816 & $r_{\beta_1}{=}0.814$, $r_{\beta_2}{=}0.818$ \\
\midrule
\multirow{3}{*}{4}
 & Static GPCM  & 0.686 & \textbf{0.983} & $r_{\beta_1}{=}0.978$, $r_{\beta_2}{=}0.987$, $r_{\beta_3}{=}0.984$ \\
 & Dynamic GPCM & \textbf{0.752} & 0.966 & $r_{\beta_1}{=}0.958$, $r_{\beta_2}{=}0.973$, $r_{\beta_3}{=}0.967$ \\
 & DEEP-GPCM    & 0.751 & 0.918 & $r_{\beta_1}{=}0.923$, $r_{\beta_2}{=}0.929$, $r_{\beta_3}{=}0.903$ \\
\midrule
\multirow{3}{*}{5}
 & Static GPCM  & 0.759 & \textbf{0.985} & $r_{\beta_1}{=}0.980$, $r_{\beta_2}{=}0.987$, $r_{\beta_3}{=}0.991$, $r_{\beta_4}{=}0.982$ \\
 & Dynamic GPCM & 0.721 & 0.966 & $r_{\beta_1}{=}0.961$, $r_{\beta_2}{=}0.971$, $r_{\beta_3}{=}0.973$, $r_{\beta_4}{=}0.956$ \\
 & DEEP-GPCM    & \textbf{0.800} & 0.912 & $r_{\beta_1}{=}0.913$, $r_{\beta_2}{=}0.929$, $r_{\beta_3}{=}0.922$, $r_{\beta_4}{=}0.885$ \\
\bottomrule
\end{tabular}
\end{table}
...
We address \textbf{RQ4} by comparing three item encoding strategies on a large-scale dataset ($Q=5000$, $K=5$, sequences of length $[400,600]$, 1000 students), where the cost of dense encoding becomes practically significant. Table~\ref{tab:recovery} reports QWK and IRT parameter recovery for LinearDecay, Separable, and SIE encodings. All models use $d_k=d_v=64$ and $N=50$ memory slots; batch size 16 (reduced from 64 to accommodate LinearDecay's $\mathrm{batch}\times S\times KQ$ input tensor). All three complete each epoch in 127--129 s, confirming that wall-clock throughput is governed by the DKVMN memory loop and not by the encoding stage. SIE achieves the best QWK (0.778) and the strongest discrimination recovery ($r_\alpha=0.654$) at only 408K parameters — approximately one-fifth of LinearDecay's 1.9M — establishing SIE as the preferred encoding when both prediction quality and psychometric recovery matter at scale.

\begin{table}[htb!]
\centering
\caption{Item encoding comparison at $Q=5000$, $K=5$. $r_\alpha$, $r_\beta$: Pearson correlations with ground-truth IRT parameters ($r_\beta$ pooled across thresholds).}
\label{tab:recovery}
\small
\setlength{\tabcolsep}{4pt}
...
\midrule
LinearDecay & 1943K & 0.762 & 0.14 & 0.022 & 0.906 \\
Separable   &   667K  & 0.773 & 0.22 & 0.246 & 0.911 \\
SIE (ours)  &   408K  & \textbf{0.778} & 0.23 & \textbf{0.654} & 0.884 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[htb!]
\centering
\caption{Ablation of loss components at $K=4$ (single seed, $Q=200$, $N=5000$).}
\label{tab:ablation_loss}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lcc}
\toprule
\textbf{Loss variant} & \textbf{QWK} & $\boldsymbol{\tau}$ \\
\midrule
Full ($\lambda_{\mathrm{focal}}{=}\lambda_{\mathrm{WOL}}{=}0.5$) & 0.683 & 0.513 \\
Focal only ($\lambda_{\mathrm{focal}}{=}1$, $\lambda_{\mathrm{WOL}}{=}0$) & 0.679 & 0.508 \\
WOL only ($\lambda_{\mathrm{focal}}{=}0$, $\lambda_{\mathrm{WOL}}{=}1$) & 0.684 & 0.516 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Monotonic threshold constraint.}
To isolate the contribution of the softplus gap construction \eqref{eqn:b_k}, we conduct a controlled ablation at $K=5$ with three conditions: (1) the baseline DEEP-GPCM model with monotonic thresholds and ordinal losses (focal + WOL), (2) an unconstrained variant that replaces the softplus gap construction with a single linear layer $\boldsymbol{\beta} = W_\beta \mathbf{q} + \mathbf{b}_\beta$ producing $K-1$ thresholds without ordering guarantees, trained with ordinal losses, and (3) the unconstrained variant trained with pure cross-entropy loss (focal\_weight $= 0$, weighted\_ordinal\_weight $= 0$) to test whether ordinal losses or the GPCM likelihood itself enforce ordering. All three models use identical architectures (Static Item Embedding, $N=5000$, $Q=200$, 15 epochs, seed 42) and differ only in the threshold parameterisation and loss composition.

Results in Table~\ref{tab:ablation_monotonic} show that removing the monotonicity constraint produces near-zero disordered step difficulties when trained with ordinal losses (0.0\% disorder), but 0.5\% disorder emerges without ordinal losses. Inspection of the learned parameters reveals that the unconstrained linear layer's bias vector converges to approximately ordered values even without ordinal losses (e.g., $[-0.90, -0.25, 0.25, 0.89]$), indicating that the GPCM likelihood provides weak implicit ordering pressure. However, ordinal losses are necessary to fully eliminate disorder. QWK degrades marginally when removing the monotonicity constraint ($\Delta\mathrm{QWK} \approx -0.002$ with ordinal losses, $-0.02$ without), but parameter recovery degrades substantially: $\bar{r}_\beta$ drops from $0.912$ (monotonic) to $0.694$ (unconstrained + ordinal) to $0.545$ (unconstrained + no ordinal), and $r_\alpha$ similarly degrades from $0.760$ to $0.600$ to $0.656$. This indicates that while unconstrained thresholds can achieve comparable predictive performance, they sacrifice interpretability — the learned $\boldsymbol{\beta}$ vectors no longer faithfully represent the ground-truth item step difficulties, undermining the psychometric validity of the extracted parameters.

\begin{table}[htb!]
\centering
\caption{Monotonicity constraint ablation at $K=5$ ($Q=200$, $N=5000$, 15 epochs, seed 42). Unconstrained models replace the softplus gap construction with a single linear layer producing $K-1$ thresholds without ordering guarantees. Recovery correlations computed on the test set.}
\label{tab:ablation_monotonic}
\small
\setlength{\tabcolsep}{3.5pt}
\begin{tabular}{lcccccc}
\toprule
\textbf{Model} & \textbf{ACC} & \textbf{QWK} & $\boldsymbol{\tau}$ & $\boldsymbol{r_\alpha}$ & $\boldsymbol{\bar{r}_\beta}$ & \textbf{Disorder (\%)} \\
\midrule
Monotonic + ordinal & 0.511 & 0.759 & 0.576 & 0.760 & 0.912 & 0.0 \\
Unconstrained + ordinal & 0.507 & 0.757 & 0.574 & 0.600 & 0.694 & 0.0 \\
Unconstrained + NO ordinal & 0.513 & 0.736 & 0.549 & 0.656 & 0.545 & 0.5 \\
\bottomrule
\end{tabular}
\end{table}
...
Table~\ref{tab:ablation_loss} reports QWK and $\tau$ for the three loss variants at $K=4$ (single seed). Differences are within $\Delta\mathrm{QWK} < 0.006$ and $\Delta\tau < 0.008$, indicating that the combined loss provides a consistent but modest improvement on this synthetic dataset.

We evaluate sequence length sensitivity at $K=4$ across three regimes: $S\in[20,80]$ (baseline), $S\in[20,100]$, and $S\in[20,40]$. QWK follows the expected direction ($0.6903$, $0.6825$, $0.6742$ respectively), with a modest drop under very short sequences. Item-level recovery ($\bar{r}_\beta \approx 0.92$ across all regimes) is stable, consistent with the person--item identification asymmetry: item parameters are estimated from $N=5000$ student interactions and are robust to individual sequence length, whereas discrimination recovery ($r_\alpha$) degrades more markedly from $0.786$ to $0.524$ as sequences shorten and item co-occurrence patterns become sparser. Ability recovery ($r_\theta$) is similarly lower at $S\in[20,40]$ ($0.158$ vs.\ $0.191$ baseline), reflecting the reduced interaction history available per student. Table~\ref{tab:seqlen_sensitivity} reports the full results.

\begin{table}[htb!]
\centering
\caption{Sequence length sensitivity at $K=4$ ($Q=200$, $N=5000$, seed 42, 15 epochs). Recovery correlations computed on the test set.}
\label{tab:seqlen_sensitivity}
\small
\setlength{\tabcolsep}{5pt}
\begin{tabular}{lcccccc}
\toprule
\textbf{Regime} & \textbf{ACC} & \textbf{QWK} & $\boldsymbol{\tau}$ & $\boldsymbol{r_\theta}$ & $\boldsymbol{r_\alpha}$ & $\boldsymbol{\bar{r}_\beta}$ \\
\midrule
$S\in[20,80]$ (baseline) & 0.529 & 0.683 & 0.513 & 0.191 & 0.715 & 0.918 \\
$S\in[20,100]$ & 0.530 & 0.690 & 0.518 & 0.168 & 0.786 & 0.927 \\
$S\in[20,40]$ & 0.523 & 0.674 & 0.505 & 0.158 & 0.524 & 0.916 \\
\bottomrule
\end{tabular}
\end{table}


\subsubsection{Proxy-Ordinality on Binary Benchmarks (RQ5)}
\label{sec:proxy_results}
Addressing \textbf{RQ5}, we evaluate whether ordinal modeling gains persist when response categories must be constructed from proxy signals rather than purpose-designed rubrics. Real KT benchmarks are predominantly binary, yet process features such as attempt counts, hint requests, and response latency encode graded evidence of understanding \citep{lu_selfkt_2024}. We derive $K=3$ ordered categories from attempt counts in ASSISTments-style interaction logs: 1 attempt ($=$ mastery), 2--3 attempts ($=$ partial understanding), 4$+$ attempts ($=$ struggle). This mapping approximates an ordinal rubric but introduces noise — guessing, rapid retries, and help-seeking behavior may violate the assumed ordering. We therefore treat this setting as a \emph{stress test} of the model's ordinal structure under imperfect category construction, and ask whether the GPCM head exploits the constructed step transitions (supporting RQ5 positively) or whether category noise erases the gains (informative null result).
...
Running the ASSISTments attempt-count converted proxy-oridnal case: DEEP-GPCM should show QWK gains over DKVMN (softmax) if ordinal category construction is consistent enough for the GPCM formulation to exploit step transitions; if attempt-count bins are too noisy, the QWK gap may narrow.
\end{pendingbox}
...
DEEP-MIRT extends binary KT to address wider applications for formative assessment practice. By estimating a dynamic ability and categorical probabilities, it differentiates partial knowledge from total misunderstanding, providing richer insights in continuous learning progressions. Furthermore, the extracted item parameters describe item-level characteristics beyond immediate predictive use. 
Unlike classical psychometric calibration, DEEP-MIRT allows online extraction of item characteristics alongside student states, which helps identify underperforming or misaligned items in real time.
This transition from a purely predictive instrument to a measurement-aware system ensures that instructional decisions are grounded in identifiable, theoretically-backed constructs such as discrimination and step difficulty \citep{heritage_formative_2007, wilson_constructing_2005}.
