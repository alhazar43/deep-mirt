...
Two findings stand out. First, the $\tau$ advantage of DEEP-GPCM over the DKVMN baselines is largest at $K=3$ and shrinks as $K$ grows, reaching near-parity at $K=5$. This is counterintuitive if one attributes the gain to the GPCM's threshold structure --- more thresholds should, if anything, help more at higher $K$. A more consistent explanation is that at low $K$, the ordinal signal per observation is weak and the GPCM's ability-centred parameterisation provides a stronger inductive bias for ranking students correctly; at high $K$, the richer per-step signal allows the DKVMN baselines to learn comparable rank structure from data alone. Second, the IRT ablation shows that sequential dynamics, not item parameterisation, drive the bulk of performance: Dynamic GPCM, which shares item tables with Static GPCM but adds a single gated ability update, recovers most of the gap to the neural models. At $K=4$ and $K=5$, Dynamic GPCM nearly matches DKVMN + Softmax on QWK despite having no distributed memory, which suggests that a lightweight recurrent IRT model is a surprisingly competitive baseline when the response scale is fine-grained enough to provide a strong per-step learning signal.
...
DEEP-GPCM recovers discrimination \emph{better} than both GPCM baselines across most conditions ($r_\alpha = 0.75$--$0.80$ vs.\ $0.69$--$0.76$ for Static and Dynamic GPCM; Table~\ref{tab:irt_recovery_k}), despite receiving no direct supervision on $\alpha$. This is the stronger finding: the DKVMN memory, trained solely on ordinal prediction, captures item-level sharpness more faithfully than models that optimise item parameters directly. Step difficulty recovery is slightly lower ($\bar{r}_\beta = 0.82$--$0.92$ for DEEP-GPCM vs.\ $0.97$--$0.99$ for the baselines), which is expected --- dedicated lookup tables have an unobstructed gradient path to each $\beta$, whereas DEEP-GPCM infers thresholds as a byproduct of sequential prediction. The absolute values remain reasonable, and per-threshold correlations deviate by at most $0.04$ from the condition mean, confirming uniform recovery across the difficulty continuum.

Figure~\ref{fig:recovery} shows that the estimated ability distributions closely track the $\mathcal{N}(0,1)$ generating prior, confirming population-level calibration. The low point-to-point $r_\theta$ is expected: the synthetic ground truth is a static scalar, whereas $\theta_t$ is a dynamic trajectory that updates with each interaction. These are different constructs --- the former is a fixed trait, the latter is an evolving belief --- and comparing them directly is a category error. The meaningful criterion for a dynamic estimator is whether $\theta_t$ moves in the right direction as evidence accumulates, which Section~\ref{sec:trajectories} addresses directly.

\begin{figure*}[htb!]
\centering
\resizebox{\linewidth}{!}{\input{kt-gpcm/outputs/deepgpcm_k5_s42/recovery_figure.pgf}}
\caption{IRT parameter recovery ($K=5$, $Q=200$, $N=5000$). Each row shows one model; columns show ability $\theta$ density, discrimination $\alpha$ (log-linked), and step difficulties $\beta_1$--$\beta_4$ (z-scored). Pearson $r$ is annotated per panel. Static GPCM: $r_\alpha=0.698$, $\bar{r}_\beta=0.985$. Dynamic GPCM: $r_\alpha=0.694$, $\bar{r}_\beta=0.965$. DEEP-GPCM: $r_\alpha=0.746$, $\bar{r}_\beta=0.912$.}
\label{fig:recovery}
\end{figure*}
\FloatBarrier
...
Figure~\ref{fig:trajectories} compares learner state trajectories across three models for four synthetic archetypes. The key contrast is between Dynamic GPCM and the two memory-based models. Dynamic GPCM's $\theta_t$ is a per-student embedding updated by a fixed recurrence, producing trajectories that are smooth but relatively flat --- the model adjusts slowly and converges to a stable estimate regardless of archetype. DEEP-GPCM and DKVMN+Softmax, sharing the same DKVMN backbone, produce trajectories that are more reactive: they track the local response pattern, rising after correct responses and dipping after errors, and separate more cleanly across archetypes. This confirms that the memory read/write mechanism, not the response head, drives the dynamic sensitivity of the learner state.

The similarity between DEEP-GPCM's $\theta_t$ and DKVMN+Softmax's expected score $\hat{s}_t$ is itself informative. Both curves track each other closely across all four archetypes, indicating that the GPCM head does not distort the latent signal encoded by the memory --- it re-expresses the same information on a psychometrically interpretable scale. The practical implication is that replacing a softmax head with a GPCM head costs nothing in trajectory quality while gaining a direct connection to item parameters: $\theta_t$ can be compared against item difficulty thresholds $\beta_k$ to determine which rubric transitions a student is ready to cross, a form of criterion-referenced feedback that $\hat{s}_t$ cannot support.

\begin{figure}[htb!]
\centering
\input{kt-gpcm/outputs/deepgpcm_k5_s42/trajectory_plots/learner_trajectories.pgf}
\caption{Learner state trajectories for four archetypes ($K=5$, $Q=200$). Curves smoothed with a 9-step rolling mean.}
