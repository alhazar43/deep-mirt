...
\title[Article Title]{MA-IRT: A Memory-Augmented Framework for Polytomous Knowledge Tracing with IRT Parameter Recovery}
...
While automated scoring systems can deliver this ordinal structure at scale in intelligent tutoring contexts \citep{shute_stealth_2011, baral_improving_2021}, most Knowledge Tracing (KT) benchmarks, evaluation protocols, and modeling paradigms still assume binary correctness. This leaves ordinal outcomes comparatively underexplored in KT, even though they are central to authentic assessment practice.
...
% We propose \textit{MA-IRT} as a \emph{framework} for extending dichotomous KT to PKT by coupling a swappable sequential backbone (e.g., memory networks or attention) with a \emph{response-aware head} grounded in psychometric item response models. In this study, we instantiate the framework for ordinal/partial-credit responses via a Generalized Partial Credit Model (GPCM) head, yielding two showcase models: \textbf{MA-GPCM}, which combines a DKVMN-style dynamic memory with a GPCM observation model, and \textbf{ATTN-GPCM}, which further integrates self-attention mechanisms to improve interaction representation and memory addressing. While our experiments emphasize ordinal PKT, the framework is designed to accommodate other polytomous formats (e.g., nominal multiple-choice and ranking) through alternative response heads.

We make the following contributions:
\begin{enumerate}
    \item \textbf{A modular framework for polytomous knowledge tracing.} We propose MA-IRT, which decouples a sequential backbone from an interchangeable psychometric response head. We instantiate the framework with a GPCM head and evaluate whether the ordinal inductive bias improves prediction (\textbf{RQ1}) and produces plausible ability trajectories (\textbf{RQ2}).
    \item \textbf{Unsupervised recovery of full IRT parameters.} We show that MA-GPCM recovers discrimination, step thresholds, and dynamic ability from interaction data alone, without ever observing ground-truth parameters during training (\textbf{RQ3}).
    \item \textbf{An empirical comparison of item representations for large item banks.} We compare three item representations (one-hot, learned, StaticItem) that share the same ordinal encoding but differ in how items are represented, finding that frozen random item vectors match learned embeddings up to $Q=2{,}000$ (\textbf{RQ4}). We also ablate the monotonicity constraint and test robustness under skewed response distributions (\textbf{RQ5}).
\end{enumerate}
...
\node[hdr] at ( 3.35, 3.70) {Value Embedding};
...
\vspace{-6pt}
\caption{MA-IRT framework at timestep $t$. Item parameters $(\alpha_j, \boldsymbol{\beta}_j)$ bypass the student state via the item embedding (dashed, top), consistent with the IRT assumption that discrimination and difficulty are student-independent. The GPCM head maps $(\theta_t, \alpha_j, \boldsymbol{\beta}_j)$ to the predicted ordinal distribution $\hat{P}(r_{t+1}=k)$.}
...
\subsubsection{Ordinal Response Kernel}
We encode an ordinal response $r_t$ with a triangular kernel over ordered categories \citep{vargas_soft_2023}.
...
\subsubsection{Item Representation}
\label{sec:item_identity}
...
\paragraph{One-hot.} The simplest approach uses a Kronecker product to bind the ordinal code to a one-hot item indicator \citep{piech_deep_2015, zhang_dynamic_2017}:
\begin{equation}
    \mathbf{x}_t = q_t \otimes \mathbf{s}_t \in \mathbb{R}^{KQ}, \qquad \mathbf{v}_t = B^{\top}\mathbf{x}_t \in \mathbb{R}^{d_v}.
\end{equation}
...
\paragraph{Learned.} A trainable dense embedding replaces the sparse one-hot item indicator. A learned item lookup $\mathbf{e}_{q_t} \in \mathbb{R}^{d_k}$ is concatenated with $\mathbf{s}_t$ and projected:
\begin{equation}\label{eqn:lie}
    \mathbf{v}_t = W_{\mathrm{proj}}[\mathbf{e}_{q_t};\;\mathbf{s}_t] + \mathbf{b}_{\mathrm{proj}} \in \mathbb{R}^{d_v}.
\end{equation}
This follows the standard DKVMN value embedding \citep{zhang_dynamic_2017} generalized from binary to ordinal responses. The projection requires $\mathcal{O}((d_k + K) \times d_v)$ parameters independent of $Q$. However, the item embeddings are fully learnable, so items with sparse observation counts may not receive enough gradient signal to learn distinctive representations.

\paragraph{StaticItem.} This option combines the fixed (non-learnable) item vectors of the one-hot approach with the dense representation of the learned approach. Each item receives a frozen random unit-norm vector at initialization, and the item vector and response code are projected through independent additive branches:
...
\end{equation}
where $\mathbf{u}_{q_t} \in \mathbb{R}^{H}$ is a \emph{frozen} (non-trainable) unit-norm random vector, $\mathbf{s}_t \in \mathbb{R}^K$ is the ordinal code from \eqref{eqn:linear_embed}, and $W_{\mathrm{item}} \in \mathbb{R}^{d_v \times H}$, $W_{\mathrm{resp}} \in \mathbb{R}^{d_v \times K}$ are learned projections shared across all items.

The factored structure separates two gradient channels: $W_{\mathrm{item}}$ is updated by item-identity signals, while $W_{\mathrm{resp}}$ is updated by response-pattern signals. Because item vectors are frozen, items cannot collapse to similar representations during training, a failure mode of learned item embeddings at large $Q$. For two independent uniformly random unit vectors in $\mathbb{R}^H$, the expected squared inner product is $1/H$ \citep{dasgupta_elementary_2003}, so cross-item gradient crosstalk in $W_{\mathrm{item}}$ decays with $H$ independently of $Q$.

The frozen dimension $H$ is set as the smallest power of two no less than $\max(128, \lfloor Q/2 \rfloor)$, clamped to $[128, 1024]$.\footnote{Concretely, $H = 128$ for $Q \le 256$, $H = 256$ for $Q \approx 500$, $H = 512$ for $Q \approx 1000$, and $H = 1024$ for $Q \ge 2000$.} The separation of frozen item vectors from learnable response projections echoes the same static-versus-dynamic design principle used in DKVMN's memory architecture (Section~\ref{sec:dkvmn}), where a fixed component provides stable structure while a learned component adapts to data.
...
$\mathbf{v}_t \in \mathbb{R}^{d_v}$ be the value vector produced by one of the item representations in Section~\ref{sec:item_identity}.
...
\subsection{MA-GPCM: an Ordinal IRT Head}
\label{sec:gpcm_head}
...
\begin{align}
    ...
    \alpha_{j} &= \exp\!\left(0.3\cdot(W_{\alpha}[\mathbf{f}_t;\mathbf{k}_t]+b_{\alpha})\right). \label{eqn:diff_net}
\end{align}
...
Although discrimination is an item-intrinsic property in the GPCM, the network in \eqref{eqn:diff_net} receives both the student-state summary $\mathbf{f}_t$ and the item key $\mathbf{k}_t$. Including the student state gives the network a richer gradient signal for learning per-item discrimination. Because $\alpha$ is estimated at every timestep, item-level discrimination is recovered by averaging per-timestep estimates across all encounters with a given item \citep{meulders_person-by-item_2004}, which marginalizes out the student-state dependence. The exponential activation ensures $\alpha_j > 0$. The scaling factor $0.3$ plays the same role as the variance of a lognormal prior in Bayesian IRT estimation \citep{de_ayala_theory_2009}, constraining the effective discrimination range without prescribing specific values.
...
% Unless otherwise stated, all results below use \textbf{single-trait} instantiations of MA-IRT; multidimensional extensions are part of future work.
...
\begin{itemize}
    ...
    \item \textit{Item thresholds:} for each item, a base difficulty is drawn from $\mathcal{N}(0,1)$, then $K{-}1$ thresholds are drawn from $\mathcal{N}(\mathrm{base},\,0.5)$ and sorted to obtain $\beta_0<\cdots<\beta_{K-2}$.
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{MA-GPCM (ours)}: DKVMN backbone with GPCM head and weighted ordinal loss \eqref{eqn:wol_loss}. Uses StaticItem by default; the choice of item representation is ablated below.
\end{itemize}
...
All models are trained with batch size 64 using Adam with learning rate $10^{-3}$ and gradient clipping to norm 1.0. MA-GPCM uses learning rate reduction on plateau with patience 5 and factor 0.8, training for 30 epochs. Baselines use patience 10 and factor 0.9, with model-specific epoch counts reflecting their different convergence rates (DKVMN+Softmax 15, Dynamic GPCM 30, Static GPCM 200). The best checkpoint is selected by validation QWK. Models are implemented in PyTorch and trained on an RTX 4060 Laptop GPU. All results are reported as mean $\pm$ standard deviation across five random seeds $\{42, 123, 7, 0, 1\}$.
...
\textbf{MA-GPCM} & $0.596 \pm 0.001$ & $\mathbf{0.594} \pm 0.001$ & $\mathbf{0.431} \pm 0.001$ & $\mathbf{0.465} \pm 0.001$ \\
\midrule
\multicolumn{5}{l}{$K=4$} \\
\cmidrule(lr){1-5}
...
\textbf{MA-GPCM} & $0.525 \pm 0.001$ & $\mathbf{0.684} \pm 0.000$ & $\mathbf{0.516} \pm 0.000$ & $\mathbf{0.609} \pm 0.001$ \\
\midrule
\multicolumn{5}{l}{$K=5$} \\
\cmidrule(lr){1-5}
...
\textbf{MA-GPCM} & $0.501 \pm 0.001$ & $\mathbf{0.761} \pm 0.001$ & $\mathbf{0.580} \pm 0.001$ & $\mathbf{0.701} \pm 0.002$ \\
\midrule
\multicolumn{5}{l}{$K=6$} \\
\cmidrule(lr){1-5}
...
\textbf{MA-GPCM} & $0.465 \pm 0.002$ & $\mathbf{0.795} \pm 0.000$ & $\mathbf{0.616} \pm 0.001$ & $\mathbf{0.805} \pm 0.001$ \\
\bottomrule
\end{tabular}
\end{table}

The baseline comparisons isolate the contribution of each architectural component. Static GPCM performs worst overall, confirming that psychometric structure alone cannot compensate for the absence of sequential dynamics. Dynamic GPCM closes much of this gap through recurrent updates but still falls short of models with distributed memory. MA-GPCM and DKVMN+Softmax share the same DKVMN backbone, so comparing them isolates the effect of the response head. DKVMN+Softmax edges ahead on ACC at every $K$, which is expected for a head optimized solely for categorical prediction. MA-GPCM must simultaneously learn interpretable IRT parameters ($\theta_t$, $\alpha_j$, $\boldsymbol{\beta}_j$) through a constrained parameterization, yet it remains competitive on ACC while surpassing DKVMN+Softmax on every ordinal metric. The GPCM head's ordinal constraint therefore acts as a useful inductive bias, and the interpretable parameterization comes at no meaningful predictive cost.


\subsubsection{Binary Compatibility on Standard KT Benchmarks}
A practical requirement for any polytomous KT framework is backward compatibility with existing binary benchmarks. When $K=2$, the GPCM category probability in~\eqref{eqn:gpcm_prob} reduces to a single logistic function of $\alpha_j(\theta_t - \beta_{j,0})$, recovering the standard 2PL IRT model. Table~\ref{tab:combined_perf} confirms that this theoretical equivalence holds in practice. MA-GPCM matches DKT and DKVMN on both ASSIST2015 and Synthetic-5, with AUC differences well within the margin of run-to-run variability. The psychometric parameterization imposes no measurable cost when ordinal structure is absent.

\begin{table}[htb!]
\centering
\caption{Binary compatibility ($K=2$). MA-GPCM matches neural baselines. Best per column in bold.}
...
MA-GPCM & \textbf{70.6}\% & \textbf{0.605} & \textbf{75.4}\% & 0.723 & 74.7\% & 0.824 \\
\bottomrule
\end{tabular}
\end{table}
...
Figures~\ref{fig:recovery_k4_student} and~\ref{fig:recovery_k4_item} visualize recovery at $K=4$, $Q=200$. The item scatter plots show that MA-GPCM's discrimination estimates follow the identity line but with visible dispersion, particularly for highly discriminating items. Threshold estimates are tighter, though still more scattered than the baseline lookup models.
...
\begin{table}[htb!]
\centering
\caption{Parameter recovery across models and $K$ at $Q=200$ (mean $\pm$ std, $N\!=\!5$ seeds). MA-GPCM recovers discrimination without supervision. Best per column in bold.}
...
\paragraph{Item representation and scalability.}
Table~\ref{tab:recovery} compares three item representations as $Q$ grows from 200 to 2{,}000. All three share the same triangular ordinal kernel; they differ only in how items are represented (one-hot, learned, or frozen random).

At $Q=200$, all three achieve comparable prediction and recovery, confirming that the GPCM extraction mechanism is robust to input representation at moderate scale. As the item bank grows, each strategy reveals a distinct profile. One-hot recovers ability ($r_\theta$) most accurately at every scale, likely because its $\mathcal{O}(KQ)$-dimensional representation gives the summary network a richer per-item signal. However, its discrimination recovery degrades and becomes increasingly variable at larger $Q$ ($r_\alpha$ std reaches $0.122$ at $Q=2{,}000$, indicating that some seeds fail to recover discrimination). Learned and StaticItem maintain stable discrimination and threshold recovery across all scales, with nearly identical mean correlations, despite having far fewer parameters (322K and 311K vs.\ 1.2M at $Q=2{,}000$).
...
\begin{table}[htb!]
\centering
\caption{Item representation and scalability at $K=4$ (mean $\pm$ std, $N\!=\!5$ seeds). Best per column in bold within each $Q$.}
\label{tab:recovery}
\setlength{\tabcolsep}{2pt}
\begin{tabular}{lccccr}
\toprule
\textbf{Repr.} & \textbf{QWK} & $r_\alpha$ & $\bar{r}_\beta$ & $r_\theta$ & \textbf{\# Params} \\
\midrule
\multicolumn{6}{l}{$Q=200$} \\
\cmidrule(lr){1-6}
StaticItem       & $0.684 \pm 0.000$ & $0.734 \pm 0.021$ & $\mathbf{0.851} \pm 0.042$ & $0.936 \pm 0.001$ & 81K \\
Learned & $\mathbf{0.686} \pm 0.000$ & $\mathbf{0.736} \pm 0.066$ & $0.849 \pm 0.030$ & $0.947 \pm 0.002$ & 91K \\
One-hot  & $0.684 \pm 0.000$ & $0.600 \pm 0.060$ & $0.796 \pm 0.052$ & $\mathbf{0.955} \pm 0.001$ & 166K \\
\midrule
\multicolumn{6}{l}{$Q=500$} \\
\cmidrule(lr){1-6}
StaticItem       & $0.700 \pm 0.001$ & $\mathbf{0.767} \pm 0.041$ & $\mathbf{0.880} \pm 0.027$ & $0.936 \pm 0.001$ & 117K \\
Learned & $\mathbf{0.702} \pm 0.001$ & $0.765 \pm 0.045$ & $0.879 \pm 0.029$ & $0.948 \pm 0.003$ & 130K \\
One-hot  & $0.699 \pm 0.001$ & $0.603 \pm 0.066$ & $0.823 \pm 0.015$ & $\mathbf{0.957} \pm 0.001$ & 339K \\
\midrule
\multicolumn{6}{l}{$Q=1{,}000$} \\
\cmidrule(lr){1-6}
StaticItem       & $0.697 \pm 0.001$ & $0.790 \pm 0.021$ & $0.875 \pm 0.025$ & $0.937 \pm 0.002$ & 181K \\
Learned & $\mathbf{0.699} \pm 0.001$ & $\mathbf{0.797} \pm 0.013$ & $\mathbf{0.880} \pm 0.022$ & $0.950 \pm 0.002$ & 194K \\
One-hot  & $0.696 \pm 0.001$ & $0.651 \pm 0.094$ & $0.787 \pm 0.051$ & $\mathbf{0.955} \pm 0.002$ & 627K \\
\midrule
\multicolumn{6}{l}{$Q=2{,}000$} \\
\cmidrule(lr){1-6}
StaticItem       & $\mathbf{0.695} \pm 0.001$ & $\mathbf{0.752} \pm 0.026$ & $\mathbf{0.889} \pm 0.016$ & $0.937 \pm 0.001$ & 311K \\
Learned & $\mathbf{0.695} \pm 0.001$ & $0.741 \pm 0.040$ & $0.881 \pm 0.012$ & $0.947 \pm 0.001$ & 322K \\
One-hot  & $0.693 \pm 0.001$ & $0.515 \pm 0.122$ & $0.854 \pm 0.025$ & $\mathbf{0.955} \pm 0.001$ & 1.2M \\
\bottomrule
\end{tabular}
\end{table}
...
Assessments in practice often exhibit skewed response distributions where higher score categories predominate. Table~\ref{tab:imbalance} evaluates MA-GPCM under progressively severe imbalance at $Q=200$, $K=4$, shifting the ability prior from $\mathcal{N}(0,1)$ to $\mathcal{N}(1.5,1)$ so that the highest category accounts for up to 58\% of responses.
