...
1. **NLL and calibration gap despite the accuracy tie.** [TESTED 2026-08-10, P1_nll_ece.md: REFUTED AS STATED -- SH wins held-out NLL in 5/6 cells (.005-.043 nats, t 3.0-19.8), accuracy tied; dkvmn-gpcm ties. Corrected statement, consistent with the routing core: the shared arm's larger effective function class implies SH weakly EXCEEDS SK in likelihood; displacement is likelihood-profitable, not merely cheap. Mechanism A stands on the probe evidence (information present, heads misread); the sign prediction here was mis-derived and is withdrawn.]
...
3. **Summary-key ablation.** [TESTED 2026-08-11: CONFIRMED, stronger than predicted -- on the spiraled reference bed, stop-gradient on the $\kappa_t$ concatenation lifts SH $b$ from .652 to .904 (past the LSTM reference .723) and SH $a$ from .752 to .853, with accuracy unchanged (.716) and the SK control flat (.950 to .962). The correction path IS the dominant displacer. Bonus finding: the displacement is BED-DEPENDENT -- on uniform-random item draws (the probe bed) the SH lag does not manifest at all ($b$ = .989 without ablation), so the mechanism requires structured (spiraled) item sequencing to generate systematic gradient pressure through the path; recorded in the ledger. Store: results/p2_toggle_sg vs p2_toggle.]
...
**Argued, with the mechanism-bearing statistic named.** The crowding ordering across architectures (Prediction 4 carries it). The identification of DKVMN's correction path as the dominant $b$-displacer is now EMPIRICAL (Prediction 3 confirmed by the spiraled-bed stop-gradient ablation, .652 to .904 at unchanged accuracy; Prediction 2b remains the untested slot-scaling half). Deposit strength proportional to route information (Prediction 6 carries it). The time-integration account of why deposits persist (M3 proves the subtraction step; persistence in the deep model is argued). The accuracy-tie argument (Prediction 1 exposes it). The misspecification battery account (Section 7, weakest).
