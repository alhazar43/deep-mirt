...
- **Current active leg: Leg B** -- core established (Stage-1 MVP and the Stage-2 clean-null iteration both done). Leg A core (WF-3) not yet run.
- **Clean-null verdict (3-seed)**: the dirty null and the magnitude inflation are NOT Laplacian artifacts. A zero-capable signed P still fabricates coupling on null data (AUC 0.852 +- 0.105 across 3 seeds, per-seed 0.778/1.000/0.778, all clear of chance, vs oracle 0.478), a prediction-loss property; and still shows 2.85x magnitude, a scale gauge (delta<->P) not recovery. So fabrication is a real finding, magnitude is dropped (report rank only), and rank recovery 0.72 holds. NOTE the shuffled-sequence floor is unreliable at K=3 (0.30 +- 0.67; only 6 off-diagonal entries, two-way orientation degeneracy), so it is NOT reported as a number; the rank claim rests on signal Spearman vs a non-structured baseline and wants a larger-K run to firm. Thesis strengthened. The neural eta_proxy is not portable across parameterizations (Jacobian artifact); operator-dominated now rests on the oracle conditional eta plus the readout>operator ordering, not eta_proxy.
- **Next (recommended)**: WF-3 (Leg A perturbation-locality probe + spaced-repetition) to complete the second axis. Hold the Leg B Stage-2 ablations (scalar-vs-vector Delta, curriculum sweep, misspecification cost) and the ELBO foil for after both axes have their core. The 3-seed null replication is done; fabrication headline is cleared for paper use.
