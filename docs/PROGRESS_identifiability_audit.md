...
| Leg B Stage-2 clean-null iteration | done | `_anchored_field_signedP.py` (signed free-symmetric P, zero-capable, spectral-radius guard): null AUC 0.815 (NOT 0.5 -- dirty null persists, NOT Laplacian artifact); mag 2.85x ~ S1 even with positivity floor removed = scale-gauge non-identifiability (not a recovered property); rank recovery Spearman 0.720 holds; shuffled floor -0.635 (stronger contrast); Stage-1 findings strengthened. Controls single-seed | GO (confirmed); fabrication = prediction-loss property; magnitude = gauge | 2026-06-29 |
...
- **The dirty-null (fabrication) is a prediction-loss property, NOT a parameterization floor.** Stage-1 Laplacian could not output zero off-diagonals (softplus > 0), so its null floor (P_hat_off 0.132, AUC 0.889) was a suspected parameterization artifact. Stage-2 signed free P CAN output exact zeros (init raw_P_off=0 -> P=I), yet on diagonal-P null data it still learns off-diagonals of 0.149, about 89% of the signal magnitude (0.168), AUC 0.815. Prediction-loss training therefore recruits the coupling operator as a prediction-smoothing device even when no coupling exists and even when the parameterization can represent its absence. The unconstrained oracle MLE on the same generator registers absence (AUC 0.478 ~ chance); the neural-vs-oracle gap (0.815 vs 0.478) is now an OPTIMIZER property, not a parameterization-bias report. This converts a suspected artifact into a genuine identifiability finding and strengthens the thesis. Caveat: the null is a single seed, replicate across 3 seeds before any paper headline. (WF-2 Stage 1 -> Stage 2)
...
- **Clean-null verdict**: the dirty null and the magnitude inflation are NOT Laplacian artifacts. A zero-capable signed P still fabricates coupling on null data (AUC 0.815 vs oracle 0.478), a prediction-loss property; and still shows 2.85x magnitude, a scale gauge (delta<->P) not recovery. So fabrication is a real finding, magnitude is dropped (report rank only), and rank recovery 0.72 holds with a sharper shuffled contrast (-0.635, gap 1.36 vs S1's 0.42). Thesis strengthened. The neural eta_proxy is not portable across parameterizations (Jacobian artifact); operator-dominated now rests on the oracle conditional eta plus the readout>operator ordering, not eta_proxy.
- **Next (recommended)**: (1) replicate the signed-P null across 3 seeds (the fabrication headline is single-seed); (2) WF-3 (Leg A perturbation-locality probe + spaced-repetition) to complete the second axis. Hold the Leg B Stage-2 ablations (scalar-vs-vector Delta, curriculum sweep, misspecification cost) and the ELBO foil for after both axes have their core.
...
- [x] diagonal-P null (fabrication control) -- oracle Stage-0 AUC 0.478 ~ 0.5 (registers absence); neural Stage-2 signed free P (zero-capable, init P=I) AUC 0.815, null P_hat_off 0.149 ~ 89% of signal. Dirty null PERSISTS without a positivity floor, so it is a prediction-loss fabrication property, not a parameterization artifact. The obvious fix (zero-capable P) is exhausted. RESOLVED (single-seed null; replicate before headline).
