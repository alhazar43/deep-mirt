...
- NEXT ACTION: EXPERIMENTS (user pivoted from writing to the rigorous experiments). The experiment + code plan is REVISED via a 5-agent ultracode workflow -> docs/experiment_blueprint.md (adversarial verdict REVISE, fixes folded in). AWAITING 4 open decisions, especially #1 the replication unit: the adversarial shows 5-fold paired Wilcoxon CANNOT reach p<0.05 (min 0.0625), and recommends the scaffold's >=15-replicate Monte-Carlo (fresh ground-truth bank per data seed) over the user's "5 CV". Then build the _p2_ fill-ins (ordinal-CE, model/engine, aggregate, realistic generator, nrm-channels) + run (Phase 2 build + Phase 3). Reuse map reinvents nothing except _p2_ordinal_ce (GPCM currently trains on WOL, banned); _p2_cv was cut. LOAD-BEARING code fact: static-decouple routes only beta to the item key, so a static-alpha-on-key head is needed for a clean slope 2x2 (Option A). The tight JEDM draft (Phase 4) exists + was previewed; Overleaf push still blocked (403).
...
### Phase 3: Experiments  [STATUS: PARTIAL -- 2026-07-01 overnight run did the core; load-bearing controls still MISSING]
...
(item params were always correct; re-scored from saved per-fold, no re-train). MISSING (load-bearing, before the paper is defensible): the multiplicative-vs-additive ablation
(top priority, makes multiplicativity necessary), the finite-vs-asymptotic budget sweep, the
oracle-clamp control, the a_star/eigenmode-inversion check. GAP: ASSISTments data absent on disk
