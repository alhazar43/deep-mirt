...
single-shift alignment, and every public accessor -- unchanged.  The decoupled
alpha key (``alpha_item_emb``) lives in the base, so it composes here with no
decoder change.  This is the swappability contract.
...
class TransformerEncoder(BaseSeqEncoder):
    ...
    def __init__(
        ...
        # Per-step token: [item_emb, resp_emb] -> hidden_dim, plus learned position.
        ...
        # alpha head, never the token input.
