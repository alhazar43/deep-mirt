...
from deep_irt.core.decoders import NRMDecoder

# nrmfmt lives outside deep_irt.core; add it for the cross-format test.
_NRMFMT = Path(__file__).resolve().parents[1] / "nrmfmt"
if str(_NRMFMT) not in sys.path:
    sys.path.insert(0, str(_NRMFMT))
...
# ---------------------------------------------------------------------------
# Cross-format transfer (binary-vs-NRM, miniature)
# ---------------------------------------------------------------------------

def test_cross_format_transfer():
    """Shared 2PL+NRM model places held-out-format items on the shared scale,
    beating the independent (never-saw-them) baseline by a clear margin."""
    ...
    gt = generate_ground_truth(
        n_items=120, n_binary_only=40, n_nrm_only=40, K=4,
        correct_option=0, seed=0,
    )
    mode_a = generate_mode_a(gt, n_respondents=600, items_per_respondent=30, seed=0)
    mode_b = generate_mode_b(gt, n_respondents=600, items_per_respondent=30, seed=0)

    joint = train_joint(gt, mode_a, mode_b, emb_dim=16, n_epochs=300,
                        lr=0.05, batch_size=8192, reg=0.01, seed=0, verbose=False)
    ib = train_indep_binary(gt, mode_a, emb_dim=16, n_epochs=300, lr=0.05,
                            batch_size=8192, reg=0.01, seed=0, verbose=False)
    inn = train_indep_nrm(gt, mode_b, emb_dim=16, n_epochs=300, lr=0.05,
                          batch_size=8192, reg=0.01, seed=0, verbose=False)

    m = compute_metrics(gt, joint.get_item_difficulty(),
                        ib.get_difficulty(), inn.get_difficulty())

    # Held-out-format placement is informative...
    assert m["joint_on_nrm_only"] >= 0.5, m
    assert m["joint_on_binary_only"] >= 0.5, m
    # ...and clearly beats the independent baseline (genuine transfer, not noise).
    assert m["nrm_only_transfer_gap"] >= 0.3, m
    assert m["binary_only_transfer_gap"] >= 0.3, m
